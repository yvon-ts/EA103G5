...
@WebServlet("/IconReaderFromDB")
public class IconReaderFromDB extends HttpServlet {
	private static final String LOAD_SPKRICON = "SELECT SPKRICON FROM SPEAKER WHERE SPKRNO = ?";
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		try {
			...
			pstmt = con.prepareStatement(LOAD_SPKRICON);
			pstmt.setString(1, spkrno);
			rs = pstmt.executeQuery();

			if (rs.next()) {
				BufferedInputStream in = new BufferedInputStream(rs.getBinaryStream("SPKRICON"));
