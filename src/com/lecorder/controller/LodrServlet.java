		...
		if ("update".equals(action)) {
			...
			try {
				...
				if (!errorMsgs.isEmpty()) {
					...
					req.setAttribute("seatList", seatList); // 好像用不到
					req.setAttribute("lecVO", lecVO);
					RequestDispatcher failureView = req.getRequestDispatcher("/front-end/lecseat/cancelSeatMem.jsp");
					failureView.forward(req, res);
					return;
				}
				LodrService lodrSvc = new LodrService();
				...
				RequestDispatcher successView = req.getRequestDispatcher("/front-end/lecseat/cancelSeatMem.jsp");
				successView.forward(req, res);
				return;
			} catch (Exception e) {
				e.printStackTrace();
