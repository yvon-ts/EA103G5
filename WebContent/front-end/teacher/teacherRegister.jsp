...
body .progress_inner #step-5:checked+div+div+div+div>.ppic5, body .progress_inner #step-4:checked+input+div+div+div+div>.ppic4,
	body .progress_inner #step-2:checked+input+input+input+div+div+div+div>.ppic2,
	body .progress_inner #step-3:checked+input+input+div+div+div+div>.ppic3,
	body .progress_inner #step-1:checked+input+input+input+input+div+div+div+div>.ppic1
	{
	top: 50%;
	left: 0px;
	opacity: 1;
...
body .progress_inner__step:before, body .progress_inner, body .progress_inner__status .box_base,
	body .progress_inner__status .box_item, body .progress_inner__status .box_ribbon,
	body .progress_inner__status .box_bow, body .progress_inner__status .box_bow__left,
	body .progress_inner__status .box_bow__right, body .progress_inner__status .box_tag,
...
body .progress_inner__bar, body .progress_inner__bar--set {
	height: 6px;
	left: 10%;
	background: repeating-linear-gradient(45deg, #1ea4ec, #1ea4ec 4px, #1f8bc5 4px,
		#1f8bc5 10px);
...
<body>
		...
		<section class="signup">
			...
			<form method="POST"
				...
				<div class='progress_inner'>
					...
					<input id='step-2' name='step' type='radio'> <input
						id='step-3' name='step' type='radio'> <input id='step-4'
						name='step' type='radio'> <input id='step-5' name='step'
						type='radio'>
								...
								<tr>
									<td>
										<ul style="color: #0099CC; font-family: 'Gochi Hand';">
											...
											<li>3.授權期間內，未經 Xducation
												事前書面同意，老師不可以將課程上架到其它付費或免費平台、翻譯重製，或作其他利用</li>
											<li>4.專屬授權不影響老師就相同主題以平面媒體或電子書之形式出版、或開設線下實體教學課程的權利</li>
											<li>5.約定「第三方服務」及第三方服務的「系統串接設定費用」，由 Xducation 與老師雙方共同負擔。</li>
											<li>6.「第三方服務」提供者包含多管道行銷平台（Shopback、LINE
												購物、蝦皮商城等），以及金流服務平台。</li>
											<li>7.課程開課日需提供「完整」課程影片給購課學員，而非分批提供，才算是「開課」。且在 Xducation
												提供課程製作服務時，老師亦需提供必要的協助，確保課程於預定開課日上架開課。</li>
 ...
 --%>
						...
						<div style="width: 100%; height: 500px;" class='tab tab-2'>
							...
							<table>
								<tr>
									...
									<th><div class="form-group" id="form-group"
											style="position: absolute; z-index: 20">
											<label id="cert"
												style="color: #0099CC; font-family: 'Gochi Hand';">點選上傳證照1<img
												class="pic"
									...
									<td><img id="license1" class="dpic"
										src="<%=request.getContextPath()%>/front-end/teacher/teacherRegister_css/images/certification.jpg"></td>
						...
						<div
							style="width: 100%; height: 500px; position: absolute; z-index: 9;"
							class='tab tab-3'>
							<h1>銀行帳號</h1>
							<input style="width: 400px; height: 30px" type="text"
								name="bankacc" id="bankacc" placeholder="只能輸入數字，限制20字"
								value="${requestScope.teacherVO.bankacc}" />
							<table>
								<tr>
									<th style="color: #0099CC; font-family: 'Gochi Hand';">我們只接受以下銀行
									</th>
								</tr>
								<tr>
									<td><img class="bankPic" id='bankpic'
										src='<%=request.getContextPath()%>/front-end/teacher/teacherRegister_css/images/CUbank.png'></td>
									<td><img class="bankPic" id='bankpic'
										src='<%=request.getContextPath()%>/front-end/teacher/teacherRegister_css/images/citi_bank.png'></td>
									<td><img class="bankPic" id='bankpic'
										src='<%=request.getContextPath()%>/front-end/teacher/teacherRegister_css/images/CTBC_Bank.png'></td>
									<td><img class="bankPic" id='bankpic'
										src='<%=request.getContextPath()%>/front-end/teacher/teacherRegister_css/images/hsbcBank.png'></td>
								...
								<tr>
									...
									<td><img class="bankPic" id='bankpic'
										src='<%=request.getContextPath()%>/front-end/teacher/teacherRegister_css/images/fubonBank.png'></td>
									<td><img class="bankPic" id='bankpic'
										src='<%=request.getContextPath()%>/front-end/teacher/teacherRegister_css/images/huananBank.png'></td>
									<td><img class="bankPic" id='bankpic'
										src='<%=request.getContextPath()%>/front-end/teacher/teacherRegister_css/images/taiwanBank.png'></td>
									<td><img class="bankPic" id='bankpic'
										src='<%=request.getContextPath()%>/front-end/teacher/teacherRegister_css/images/firstBank.png'></td>
								</tr>
						...
						<div style="width: 100%; height: 500px;" id="applyouter"
							class='tab tab-4'>
							<h1 id="applytext">確認申請</h1>
							<table>
								<tr>
									<th
										style="padding-top: 50px; font-size: 20px; color: #0099CC; font-family: 'Gochi Hand'">
										提醒:確認選擇後，該帳號不得再次更改「開課身份」，未來若有任何需要調整、協助的地方，歡迎聯繫你的 Xducation
										專屬專案企劃夥伴，或透過右下角客服告訴我們 。</th>
								...
								<tr>
									<td style="padding-right: 160px; padding-bottom: 150px;">
										...
										type='hidden' name='memno' value='${loginMembersVO.memno}'>
										<input id="apply" style="position: absolute; z-index: 12;"
										type="submit" name="signup" class="form-submit" value="提交" />
